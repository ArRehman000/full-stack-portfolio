...
const INFO_GRID = [
  ...
  { label: "Availability", value: "Open to work", icon: Clock, valueClassName: "text-green-500" },
      ...
      <div className="max-w-5xl">
        ...
        >
          ...
          <motion.div variants={fadeIn} className="flex flex-col gap-6">
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-purple-400">About me</span>
              </div>
              <h2 className="mt-4 text-3xl font-medium leading-tight text-[#f0eeff]">
                Passionate about building <span className="text-purple-400">great products</span>
              </h2>
            </div>

            <div className="space-y-4 text-sm leading-relaxed text-zinc-500">
              <p>
                I'm a Full Stack Developer with 3 years of professional experience turning ideas into production-ready web
                applications. I've worked across startups and agencies, contributing to everything from AI-powered platforms to
                Web3 dApps.
              </p>
              <p>
                My strength lies in owning features end-to-end — architecting the backend, designing the database schema,
                and building the frontend interface, all with a focus on performance and clean code.
              </p>
            </div>
            ...
            <div className="mt-4 flex gap-4 rounded-xl border border-purple-500/20 bg-purple-950/20 p-5 group">
              ...
              <div>
                <h3 className="text-sm font-medium text-purple-300">Currently at Petalnex</h3>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500">
                  Building a full HRMS system with MERN stack — modules for attendance, leaves, tasks, meetings and
                  notifications.
                </p>
              </div>
            </div>
          ...
          <motion.div variants={fadeIn} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {INFO_GRID.map((item) => {
                ...
                const Card = (
                  ...
                  >
                    <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-600 font-mono">{item.label}</p>
                    <p className={cn("mt-1.5 text-sm font-medium text-zinc-300", item.valueClassName)}>
                      {item.value}
                    </p>
                  </div>
                );
            ...
            <div className="flex items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 transition-colors hover:bg-white/[0.04]">
              ...
              <div>
                <h3 className="text-sm font-medium text-zinc-300">BSc Software Engineering</h3>
                <p className="text-xs text-zinc-500">COMSATS University Islamabad, Abbottabad</p>
                <p className="mt-1 text-[11px] text-zinc-600">Graduated July 2023</p>
              </div>
            </div>
          </motion.div>
        </motion.section>
        ...
        >
          <div>
            <div className="flex items-center gap-3">
              <div className="h-[1px] w-8 bg-purple-500" />
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-purple-400">Skills & Tech Stack</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {SKILL_CARDS.map((skill) => (
              ...
              >
                <div className={cn("flex h-9 w-9 items-center justify-center rounded-[9px] mb-4 transition-transform group-hover:scale-110", skill.iconBg)}>
                  <skill.icon size={18} className={skill.iconColor} />
                </div>
                <h3 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 mb-3">{skill.category}</h3>
